...
function buildStats(counts, total) {
    const stateStats = STATE_ORDER.map(s => `<span class="stat stat--${s}">${STATE_LABEL[s]}: ${counts[s] ?? 0}</span>`).join('');
    return `${stateStats}<span class="stat stat--total">Total: ${total}</span>`;
}
function buildColumn(state, items) {
...
function buildCard(item, events) {
    const actionButtons = events.map(e => `<button class="btn btn--action btn--${e}" data-id="${item.id}" data-event="${e}">${e}</button>`).join('');
    return `
    <div class="card">
